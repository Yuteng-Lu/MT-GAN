...
# the range of the normalization (line 74) is changed from [0, 1] to [-1, 1]
...
def load_real_samples():
    ...
    x = x.astype('float32')
    # scale from [0,255] to [0,1], mutation changes this to a scale from [0,255] to [-1,1]
    x = (x - 127.5) / 127.5
    return x
...
def summarize_performance(epoch, g_model, d_model, dataset, latent_dim, n_samples=100, save=True):
    ...
    if save:
        ...
        filename = 'cn_generator_model_%03d.h5' % (epoch + 1)
        g_model.save(filename)
        filename = 'cn_discriminator_model_%03d.h5' % (epoch + 1)
