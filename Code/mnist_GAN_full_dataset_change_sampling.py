# change sampling
# Conv2DTranspose is replaced by UpSampling2D at line 40, 43
...
from keras.layers import Dense, Reshape, Flatten, Conv2D, UpSampling2D, LeakyReLU, Dropout
...
def save_plot(examples, epoch, n=10):
    ...
    # save plot to file
    filename = 'cs_generated_plot_e%03d.png' % (epoch + 1)
...
def summarize_performance(epoch, g_model, d_model, dataset, latent_dim, n_samples=100, save=True):
    ...
    if save:
        ...
        filename = 'cs_generator_model_%03d.h5' % (epoch + 1)
        g_model.save(filename)
        filename = 'cs_discriminator_model_%03d.h5' % (epoch + 1)
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=256):
    ...
    for i in range(n_epochs):
        ...
        if (i + 1) % 10 == 0:
            # p is figured out by line 179 in the last iteration, i.e, the last line of the output
